...
subroutine gammarhot(rho,t,gamm)
    ...
    real(8), dimension(:), allocatable :: species,dlnzdlnt,d2lnzdlnt,dlnndlnrho,dlnndlnt
    allocate(species(5),dlnzdlnt(5),d2lnzdlnt(5),dlnndlnrho(5),dlnndlnt(5))
    ...
    deallocate(species,dlnzdlnt,d2lnzdlnt,dlnndlnrho,dlnndlnt)
...
function eos_h_cv(rho,t,temp1,temp2,temp3,temp4)
    real(8) :: eos_h_cv,rho,t,t1,t2,t3,t4,xi_h2,xi_h,xi_hion,xi_e,xi_he,xi(5)
    real(8), optional :: temp1,temp2,temp3,temp4
    real(8), dimension(:), allocatable :: species,dlnndlnt,dndt
    allocate(species(5),dlnndlnt(5),dndt(5))
    ...
    eos_h_cv=dot_product(dndt,xi)+sum(species)*1.5d0*kb
    deallocate(species,dlnndlnt,dndt)
end function eos_h_cv
...
function srhot(rho,t,temp1,temp2,temp3,temp4)
    ...
    real(8), optional :: temp1,temp2,temp3,temp4
    real(8), dimension(:), allocatable :: species,dlnzdlnt,d2lnzdlnt,mu,si
    allocate(species(5),dlnzdlnt(5),d2lnzdlnt(5),mu(5),si(5))
    if (present(temp1)) then
        t1=temp1
        t2=temp2
        t3=temp3
    ...
    deallocate(species,dlnzdlnt,d2lnzdlnt,mu,si)
end function srhot

function srhot_per_particle(rho,t,temp1,temp2,temp3,temp4)
    real(8) :: srhot,rho,t,srhot_per_particle,n,t1,t2,t3,t4
    real(8), optional :: temp1,temp2,temp3,temp4
    real(8), dimension(:), allocatable :: species,dlnzdlnt,d2lnzdlnt,mu
    allocate(species(5),dlnzdlnt(5),d2lnzdlnt(5),mu(5))
    if (present(temp1)) then
        t1=temp1
        t2=temp2
        t3=temp3
    ...
    deallocate(species,dlnzdlnt,d2lnzdlnt,mu)
...
subroutine calsrhost(rho,t,species,dlnndlnrho,dlnndlnt,dlnzdlnt,d2lnzdlnt,srho,st)
    ...
    real(8), dimension(:), allocatable :: dsdrho,dsdt,dndrho,dndt,mu,species,dlnndlnrho,dlnndlnt,dlnzdlnt,d2lnzdlnt
    integer :: i
    allocate(dsdrho(5),dsdt(5),dndrho(5),dndt(5),mu(5))
    ...
    do i=1,5
        ...
        !if (isnan(dsdrho(i))) then
        !    print *, 'calsrhost wrong, check eos limits'
        !    write(*,'(6A18)'),'number density','dlnndlnrho','dlnzdlnt','chem potential','temperature','density'
        !    write(*,'(12ES18.6E3)') species(i),dlnndlnrho(i),dlnzdlnt(i),mu(i),t,rho
        !    stop
        !end if
    end do
    srho=sum(dsdrho)
    st=sum(dsdt)
    deallocate(dsdrho,dsdt,dndrho,dndt,mu)
end subroutine calsrhost

subroutine calprhopt(species,dlnndlnrho,dlnndlnt,rho,t,prho,pt)
    real(8) :: rho,t,prho,pt,ntot
    real(8), dimension(:), allocatable :: species,dlnndlnrho,dlnndlnt
    integer :: i
    ntot=sum(species)
    prho=0d0
    pt=0d0
...
subroutine caldlnz(t,dlnzdlnt,d2lnzdlnt)
    real(8) :: t
    real(8), dimension(:), allocatable :: dlnzdlnt,d2lnzdlnt
    dlnzdlnt(1)=1.5d0
...
subroutine calmu(rho,t,species,mu,temp1,temp2,temp3,temp4)
    ...
    real(8), optional :: temp1,temp2,temp3,temp4
    real(8), dimension(:), allocatable :: species,mu
    if (present(temp1)) then
        t1=temp1
        t2=temp2
        t3=temp3
...
subroutine caldlnndlnrho(rho,t,species,dlnndlnrho,temp1,temp2,temp3,temp4)
    ...
    real(8), optional :: temp1,temp2,temp3,temp4
    real(8), dimension(:), allocatable :: species,dlnndlnrho
    if (present(temp1)) then
        t1=temp1
        t2=temp2
        t3=temp3
...
subroutine caldndt(rho,t,species,dndt)
    real(8) :: rho,t
    real(8), dimension(:), allocatable :: species,dndt,dlnndlnt
    integer :: i
    allocate(dlnndlnt(5))
...
subroutine caldlnndlnt(rho,t,species,dlnndlnt,temp1,temp2,temp3,temp4)
    ...
    real(8), optional :: temp1,temp2,temp3,temp4
    real(8), dimension(:), allocatable :: species,dlnndlnt
    !partition of energy
    ph2=1.5d0
...
function zh(t)
    ...
    zh=(4d0*sqrt(8d0)*pi**1.5*(kb*mh*t)**1.5)/(exp(dish/(2d0*kb*t))*h_planck**3)
end function zh

function ze(t)
    real(8) :: ze,t
    ze=(2d0*sqrt(8d0)*pi**1.5*(kb*me*t)**1.5)/h_planck**3
end function ze

function zhion(t)
    real(8) :: zhion,t,mhion
    mhion=mh-me
    zhion=(2d0*sqrt(8d0)*pi**1.5*(kb*mhion*t)**1.5)/(exp((dish+2d0*ionh)/(2d0*kb*t))*h_planck**3)
end function zhion

function zh2(t)
    real(8) :: zh2,t
    zh2=(4d0*sqrt(8d0)*pi**1.5*(kb*mh2*t)**1.5)/h_planck**3
end function zh2

function zhe(t)
    real(8) :: zhe,t
    zhe=(4d0*sqrt(8d0)*pi**1.5*(kb*mhe*t)**1.5)/h_planck**3
end function zhe
