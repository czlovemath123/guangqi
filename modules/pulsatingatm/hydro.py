...
while os.path.isfile(filename):
    ...
    fig,axes=plt.subplots(3,figsize=(8,10),sharex=True,squeeze=True)
    axes[0].plot(x_axis_au,temp,'k-',linewidth=1,label=r'$T_{gas}$')
    ...
    axes[1].plot(x_axis_au,v/1e5,'k-',linewidth=1,label='v')
    axes[1].legend(loc=0)
    ...
    axes[2].plot(x_axis_au,rho,'k-',linewidth=1,label=r'$\rho$')
    axes[2].legend(loc=0)
    ...
    axes[2].set_ylabel(r'$g\cdot cm^{-3}$')



    plt.tight_layout()
    plt.subplots_adjust(hspace=.0)
    plt.savefig(cwd+'/pulsation'+filenumber+'.png',dpi=100)
    plt.close()
    k=k+1
    filenumber=str(k).zfill(5)
    filename=filehead+filenumber+'.h5'
